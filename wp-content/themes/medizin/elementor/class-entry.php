...
final class Entry {
	 ...
	 */
	const MINIMUM_ELEMENTOR_VERSION = '3.1.4';
	...
	public function check_validate() {
		 ...
		 * WPML supported.
		 */
		require_once MEDIZIN_ELEMENTOR_DIR . '/wpml/class-wpml-translatable-nodes.php';

		require_once MEDIZIN_ELEMENTOR_DIR . '/class-widget-utils.php';
		require_once MEDIZIN_ELEMENTOR_DIR . '/class-widget-init.php';
		require_once MEDIZIN_ELEMENTOR_DIR . '/class-control-init.php';
	}

	function elementor_editor_scripts() {
		wp_enqueue_style( 'medizin-elementor-editor', MEDIZIN_ELEMENTOR_ASSETS . '/css/editor.css' );
	}
	...
	public function admin_notice_missing_main_plugin() {
		...
		/* translators: 1: Plugin name 2: Elementor */
			esc_html__( '"%1$s" requires "%2$s" to be installed and activated.', 'medizin' ),
			'<strong>' . esc_html__( 'Medizin', 'medizin' ) . '</strong>',
			'<strong>' . esc_html__( 'Elementor', 'medizin' ) . '</strong>'
		);
	...
	public function admin_notice_minimum_php_version() {
		...
		/* translators: 1: Plugin name 2: PHP 3: Required PHP version */
			esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.', 'medizin' ),
			'<strong>' . esc_html__( 'Medizin', 'medizin' ) . '</strong>',
			'<strong>' . esc_html__( 'PHP', 'medizin' ) . '</strong>',
