...
class Widget_List extends Base {
	...
	public function get_title() {
		return esc_html__( 'Modern Icon List', 'medizin' );
	}
	...
	private function add_list_section() {
		$this->start_controls_section( 'list_section', [
			'label' => esc_html__( 'Icon List', 'medizin' ),
		] );
		...
		$this->add_control( 'style', [
			...
			'default'      => '',
			'options'      => [
				''            => esc_html__( 'Normal', 'medizin' ),
				'icon-border' => esc_html__( 'Icon Border', 'medizin' ),
			],
			'prefix_class' => 'medizin-list-style-',
		] );
		...
		$this->add_control( 'layout', [
			...
			'options'      => [
				'block'   => [
					'title' => esc_html__( 'Default', 'medizin' ),
					'icon'  => 'eicon-editor-list-ul',
				],
				'inline'  => [
					'title' => esc_html__( 'Inline', 'medizin' ),
					'icon'  => 'eicon-ellipsis-h',
				],
				'columns' => [
					'title' => esc_html__( 'Columns', 'medizin' ),
					'icon'  => 'eicon-columns',
				],
			],
			'prefix_class' => 'medizin-list-layout-',
		] );

		$this->add_control( 'icon', [
			'label'       => esc_html__( 'Default Icon', 'medizin' ),
			'description' => esc_html__( 'Choose default icon for all items.', 'medizin' ),
			'type'        => Controls_Manager::ICONS,
		] );

		$this->add_control( 'icon_vertical_alignment', [
			'label'                => esc_html__( 'Icon Alignment', 'medizin' ),
			'type'                 => Controls_Manager::CHOOSE,
	...
	private function add_styling_section() {
		...
		$this->add_control( 'items_vertical_spacing', [
			'label'      => esc_html__( 'Items Spacing', 'medizin' ),
			...
			'selectors'  => [
				'{{WRAPPER}}.medizin-list-layout-block .item + .item, {{WRAPPER}}.medizin-list-layout-columns .item:nth-child(2) ~ .item' => 'margin-top: {{SIZE}}{{UNIT}};',
				'{{WRAPPER}}.medizin-list-layout-inline .item'                                                                             => 'margin-bottom: {{SIZE}}{{UNIT}};',
			],
		] );
		...
		$this->add_control( 'width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .medizin-list' => 'width: {{SIZE}}{{UNIT}};',
			],
		] );
	...
	private function add_icon_style_section() {
		...
		$this->add_responsive_control( 'min_width', [
			'label'          => esc_html__( 'Min Width', 'medizin' ),
			'type'           => Controls_Manager::SLIDER,
			'default'        => [
				'unit' => 'px',
			],
		...
		$this->add_control( 'icon_marker_color', [
			'label'     => esc_html__( 'Icon Marker', 'medizin' ),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}}.medizin-list-style-icon-border .icon' => 'border-color: {{VALUE}};',
			],
			'condition' => [
				'style' => [
					'icon-border',
		...
		$this->add_control( 'hover_icon_marker_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}}.medizin-list-style-icon-border .link:hover .icon' => 'border-color: {{VALUE}};',
			],
			'condition' => [
				'style' => [
					'icon-border',
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'medizin-list' );

		$global_icon_html = '';
		if ( ! empty ( $settings['icon']['value'] ) ) {
			$global_icon_html = '<div class="medizin-icon icon">' . $this->get_render_icon( $settings, $settings['icon'], [ 'aria-hidden' => 'true' ], false, 'icon' ) . '</div>';
		}
		?>
	...
	protected function _content_template() {
		...
		if ( '' !== settings.icon.value ) {
			...
			global_icon_html += '<div class="medizin-icon icon">' + globalIconHTML.value + '</div>';
		}
		#>
