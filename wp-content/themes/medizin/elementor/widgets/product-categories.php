...
class Widget_Product_Categories extends Base {
	...
	public function get_title() {
		return esc_html__( 'Product Categories', 'medizin' );
	}
	...
	private function add_query_section() {
		...
		$this->add_control( 'source', [
			...
			'options'     => [
				...
				'current_subcategories' => esc_html__( 'Current Subcategories', 'medizin' ),
			],
			'label_block' => true,
		] );
		...
		$this->add_control( 'number', [
			'label'     => esc_html__( 'Categories Count', 'medizin' ),
			'type'      => Controls_Manager::NUMBER,
			'default'   => '4',
			'condition' => [
				'source!' => 'by_id',
	...
	protected function render() {
		...
		$this->add_render_attribute( 'grid-wrapper', 'class', 'medizin-grid-wrapper medizin-product-categories' );

		$this->add_render_attribute( 'content-wrapper', 'class', 'medizin-grid lazy-grid' );

		if ( 'metro' === $settings['layout'] ) {
			$this->add_render_attribute( 'grid-wrapper', 'class', 'medizin-grid-metro' );
		}
	...
	private function print_grid() {
		$settings   = $this->get_settings_for_display();
		$image_size = \Medizin_Image::elementor_parse_image_size( $settings, '300x300' );
		...
		foreach ( $this->terms as $term ) {
			...
			?>
			<div class="grid-item">
				<a href="<?php echo esc_url( $link ); ?>" class="product-cat-wrapper medizin-box cat-link link-secret">
