...
class Widget_Blog extends Posts_Base {
	...
	public function get_title() {
		return esc_html__( 'Blog', 'medizin' );
	}
	...
	private function add_layout_section() {
		...
		$this->add_control( 'layout', [
			...
			'options' => [
				...
				'zigzag'            => esc_html__( 'Zigzag', 'medizin' ),
				'list-01'           => esc_html__( 'List - Large Image', 'medizin' ),
				'list-02'           => esc_html__( 'List - Left Image', 'medizin' ),
				'list-03'           => esc_html__( 'List - Simple', 'medizin' ),
				'one-left-featured' => esc_html__( '1 Left Featured', 'medizin' ),
			],
			'default' => 'grid',
		] );
	...
	private function add_banners_section() {
		$this->start_controls_section( 'banners_section', [
			'label'     => esc_html__( 'Banners', 'medizin' ),
			'condition' => [
				'layout' => [ 'masonry' ],
			],
		] );
		...
		$banner_repeater->add_control( 'position', [
			'label' => esc_html__( 'After Post Number', 'medizin' ),
			'type'  => Controls_Manager::NUMBER,
			'step'  => 1,
		] );
	...
	private function add_caption_popover() {
		...
		$this->add_control( 'show_caption_read_more', [
			'label'     => esc_html__( 'Read More', 'medizin' ),
			'type'      => Controls_Manager::SWITCHER,
			'label_on'  => esc_html__( 'Show', 'medizin' ),
			'label_off' => esc_html__( 'Hide', 'medizin' ),
			'default'   => 'yes',
		...
		$this->add_control( 'read_more_text', [
			'label'     => esc_html__( 'Read More Text', 'medizin' ),
			'type'      => Controls_Manager::TEXT,
			'default'   => esc_html__( 'Read More', 'medizin' ),
			'condition' => [
		...
		$this->add_control( 'show_caption_meta', [
			...
			'options'     => [
				...
				'views'    => esc_html__( 'View Count', 'medizin' ),
				'comments' => esc_html__( 'Comment Count', 'medizin' ),
			],
		] );
	...
	private function add_overlay_popover() {
		...
		$this->add_control( 'overlay_style', [
			...
			'options' => [
				...
				'float-02' => esc_html__( 'Float-02', 'medizin' ),
			],
			'default' => 'float',
		] );
		...
		$overlay_meta_repeater->add_control( 'meta', [
			'label'       => esc_html__( 'Select Meta', 'medizin' ),
			'label_block' => true,
			'type'        => Controls_Manager::SELECT,
			'default'     => 'date',
			'options'     => [
				'author'   => esc_html__( 'Author', 'medizin' ),
				'date'     => esc_html__( 'Date', 'medizin' ),
				'comments' => esc_html__( 'Comments', 'medizin' ),
			],
		] );

		$this->add_control( 'overlay_meta_data', [
			'label'         => esc_html__( 'Meta Data', 'medizin' ),
			'type'          => Controls_Manager::REPEATER,
			'fields'        => $overlay_meta_repeater->get_controls(),
			'title_field'   => '{{{ meta }}}',
			'classes'       => 'medizin-control-repeater-title-capitalize',
			'prevent_empty' => false,
		] );
	...
	private function add_thumbnail_style_section() {
		...
		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			...
			'selector' => '{{WRAPPER}} .medizin-box:hover .post-thumbnail',
		] );

		$this->add_group_control( Group_Control_Css_Filter::get_type(), [
			'name'     => 'css_filters_hover',
			'selector' => '{{WRAPPER}} .medizin-box:hover .post-thumbnail img',
		] );
		...
		$this->add_control( 'thumbnail_opacity_hover', [
			...
			'selectors' => [
				'{{WRAPPER}} .medizin-box:hover .post-thumbnail img' => 'opacity: {{SIZE}};',
			],
		] );
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', [
			'medizin-grid-wrapper medizin-blog',
			'medizin-blog-' . $settings['layout'],
		] );

		if ( ! empty( $settings['show_caption'] ) && 'yes' === $settings['show_caption'] ) {
			$this->add_render_attribute( 'wrapper', 'class', 'medizin-blog-caption-style-' . $settings['caption_style'] );
		}

		if ( ! empty( $settings['show_overlay'] ) && 'yes' === $settings['show_overlay'] ) {
			$this->add_render_attribute( 'wrapper', 'class', 'medizin-blog-overlay-style-' . $settings['overlay_style'] );
		}
		...
		<div <?php $this->print_attributes_string( 'wrapper' ); ?>>
			<?php if ( $query->have_posts() ) : ?>
				...
				?>
				<input type="hidden"
				       class="medizin-query-input" <?php echo 'value="' . $medizin_grid_query . '"'; ?>/>
