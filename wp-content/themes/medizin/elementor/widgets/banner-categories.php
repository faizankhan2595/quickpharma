...
class Widget_Banner_Categories extends Base {
	...
	public function get_title() {
		return esc_html__( 'Banner Categories', 'medizin' );
	}
	...
	private function add_content_section() {
		...
		$this->add_control( 'cat_slugs', [
			'label'     => __( 'Select Categories', 'medizin' ),
			'type'      => Controls_Manager::SELECT,
			'default'   => '',
			'multiple'  => true,
			'options'   => self::medizin_get_categories(),
		] );
	...
	private function add_box_style_section() {
		...
		$this->add_responsive_control( 'box_radius', [
			'label'      => esc_html__( 'Border box radius', 'medizin' ),
			'type'       => Controls_Manager::DIMENSIONS,
			'size_units' => 'px',
			'selectors'  => [
				'{{WRAPPER}} .medizin-image' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );
		...
		$this->add_responsive_control( 'box_max_width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .medizin-box' => 'max-width: {{SIZE}}{{UNIT}};',
			],
		] );
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'medizin-banner-categories medizin-box' );
			...
			<?php if ( ! empty( $settings['image']['url'] ) ) : ?>
				<div class="medizin-image image">
					...
					<?php
						...
						$array_cat             = self::medizin_get_categories();
						$array_cat_link              = self::medizin_get_link_categories();
						$array_cat_count             = self::medizin_get_categories_count();
						$count = $array_cat_count[$id];

						if ( 1 === $count ) {
							$text_after = esc_attr('item', 'medizin');
						} else {
							$text_after = esc_attr('items', 'medizin');
						}
	...
	protected function _content_template() {
		...
		?>
		<#
		view.addRenderAttribute( 'wrapper', 'class', 'medizin-banner-categories medizin-box' );
		...
		if ( settings.image.url ) {
			...
			imageHTML = '<div class="medizin-image image"><img ' + view.getRenderAttributeString( 'image' ) + ' /></div>';
		}
		#>
	...
	protected function medizin_get_categories() {
		$orderby = 'name';
		$order = 'asc';
		$hide_empty = false ;
		$cat_args = array(
	...
	protected function medizin_get_link_categories() {
		$orderby = 'name';
		$order = 'asc';
		$hide_empty = false ;
		$cat_args = array(
	...
	protected function medizin_get_categories_count() {
		$orderby = 'name';
		$order = 'asc';
		$hide_empty = false ;
		$cat_args = array(
