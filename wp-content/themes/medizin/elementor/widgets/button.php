...
class Widget_Button extends Base {
	...
	public function get_title() {
		return esc_html__( 'Advanced Button', 'medizin' );
	}
	...
	private function add_button_settings_section() {
		...
		$this->add_control( 'text', [
			...
			'default'     => esc_html__( 'Click here', 'medizin' ),
			'placeholder' => esc_html__( 'Click here', 'medizin' ),
		] );
		...
		$this->add_control( 'action', [
			'label'       => esc_html__( 'Action', 'medizin' ),
			'description' => esc_html__( 'To make smooth scroll action work then input button link like this: #about-us-section.', 'medizin' ),
			'type'        => Controls_Manager::SELECT,
			'default'     => '',
			'options'     => [
				''              => esc_html__( 'Normal', 'medizin' ),
				'smooth_scroll' => esc_html__( 'Smooth scroll to a section', 'medizin' ),
				'popup_video'   => esc_html__( 'Open Popup Video', 'medizin' ),
			],
		] );
		...
		$this->add_control( 'button_css_id', [
			'label'       => esc_html__( 'Button ID', 'medizin' ),
			'type'        => Controls_Manager::TEXT,
			'dynamic'     => [
				'active' => true,
			],
			'default'     => '',
			'title'       => esc_html__( 'Add your custom id WITHOUT the Pound key. e.g: my-id', 'medizin' ),
			'label_block' => false,
			'description' => wp_kses( __( 'Please make sure the ID is unique and not used elsewhere on the page this form is displayed. This field allows <code>A-z 0-9</code> & underscore chars without spaces.', 'medizin' ), 'medizin-default' ),
			'separator'   => 'before',
		] );
	...
	private function add_button_badge_section() {
		...
		$this->add_control( 'badge_text', [
			...
			'placeholder' => esc_html__( '$59', 'medizin' ),
		] );
	...
	private function add_wrapper_section() {
		...
		$this->add_control( 'rounded', [
			'label'      => esc_html__( 'Rounded', 'medizin' ),
			'type'       => Controls_Manager::DIMENSIONS,
			'size_units' => [ 'px', '%' ],
			'selectors'  => [
	...
	private function add_skin_section() {
		...
		$this->add_control( 'hover_animation', [
			'label' => esc_html__( 'Hover Animation', 'medizin' ),
