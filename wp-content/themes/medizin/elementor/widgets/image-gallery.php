...
class Widget_Image_Gallery extends Base {
	...
	public function get_title() {
		return esc_html__( 'Image Gallery', 'medizin' );
	}
	...
	private function add_content_section() {
		...
		$this->add_control( 'gallery', [
			'label'      => esc_html__( 'Add Images', 'medizin' ),
	...
	private function add_layout_section() {
		...
		$this->add_control( 'lightbox_enable', [
			'label'        => esc_html__( 'Lightbox', 'medizin' ),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
		] );
	...
	protected function render() {
		...
		$this->add_render_attribute( 'grid-wrapper', 'class', 'medizin-grid-wrapper' );
		...
		if ( ! empty( $settings['lightbox_enable'] ) ) {
			$this->add_render_attribute( 'grid-wrapper', 'class', 'medizin-light-gallery' );
		}
		?>
	...
	private function print_grid( array $settings ) {
		$image_size = \Medizin_Image::elementor_parse_image_size( $settings, '600x600' );
	...
	private function print_image( $image, $image_size ) {
		...
		if ( ! empty( $settings['lightbox_enable'] ) ) {
			...
			$image_full = \Medizin_Image::get_attachment_info( $image['id'] );
