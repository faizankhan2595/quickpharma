...
class Widget_Image_Box extends Base {
	...
	public function get_title() {
		return esc_html__( 'Image Box', 'medizin' );
	}
	...
	private function add_image_box_section() {
		$this->start_controls_section( 'image_section', [
			'label' => esc_html__( 'Image Box', 'medizin' ),
		] );
		...
		$this->add_control( 'description_text', [
			...
			'default'     => esc_html__( 'Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'medizin' ),
	...
	private function add_box_style_section() {
		...
		$this->add_responsive_control( 'box_padding', [
			...
			'size_units' => [ 'px', '%', 'em' ],
			'selectors'  => [
				'{{WRAPPER}} .medizin-box' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );
		...
		$this->add_group_control( Group_Control_Background::get_type(), [
			'name'     => 'box_hover',
			'selector' => '{{WRAPPER}} .medizin-box:before',
		] );
	...
	private function add_image_style_section() {
		...
		$this->add_responsive_control( 'image_wrap_height', [
			...
			'selectors' => [
				'{{WRAPPER}} .medizin-image' => 'min-height: {{SIZE}}{{UNIT}};',
			],
		] );

		$this->add_responsive_control( 'image_space_top', [
			'label'     => esc_html__( 'Offset Top', 'medizin' ),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'min' => 0,
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'tm-image-box medizin-box' );
	...
	protected function _content_template() {
		...
		?>
		<#
		view.addRenderAttribute( 'wrapper', 'class', 'tm-image-box medizin-box' );
	...
	private function print_description (array $settings) {
		...
		<div <?php $this->print_render_attribute_string('description_text'); ?>>
			<?php echo wp_kses($settings['description_text'], 'medizin-default'); ?>
		</div>
		<?php
	}
}
