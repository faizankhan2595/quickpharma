...
class Widget_Accordion extends Base {
	...
	public function get_title() {
		return esc_html__( 'Modern Accordion/Toggle', 'medizin' );
	}
	...
	public function get_script_depends() {
		return [ 'medizin-widget-accordion' ];
	}
	...
	private function add_content_section() {
		...
		$this->add_control( 'style', [
			...
			'prefix_class' => 'medizin-accordion-style-',
		] );
		...
		$this->add_control( 'type', [
			...
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'accordion' => esc_html__( 'Accordion', 'medizin' ),
				'toggle'    => esc_html__( 'Toggle', 'medizin' ),
			],
			'default' => 'accordion',
		] );

		$this->add_control( 'active_first_item', [
			'label'              => esc_html__( 'Active First Item', 'medizin' ),
		...
		$repeater->add_control( 'title', [
			...
			'default'     => esc_html__( 'Accordion Title', 'medizin' ),
			'label_block' => true,
			'dynamic'     => [
				'active' => true,
			],
		...
		$repeater->add_control( 'content', [
			...
			'default' => esc_html__( 'Accordion Content', 'medizin' ),
		] );
		...
		$this->add_control( 'items', [
			...
			'default'     => [
				[
					'title'   => esc_html__( 'Accordion #1', 'medizin' ),
				...
				[
					'title'   => esc_html__( 'Accordion #2', 'medizin' ),
					'content' => esc_html__( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'medizin' ),
				],
				[
					'title'   => esc_html__( 'Accordion #3', 'medizin' ),
					'content' => esc_html__( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'medizin' ),
				],
			],
			'title_field' => '{{{ title }}}',
		...
		$this->add_control( 'active_icon', [
			'label'       => esc_html__( 'Active Icon', 'medizin' ),
			'type'        => Controls_Manager::ICONS,
			'default'     => [
		...
		$this->add_control( 'title_html_tag', [
			'label'     => esc_html__( 'Title HTML Tag', 'medizin' ),
			'type'      => Controls_Manager::SELECT,
			'options'   => [
				'h1'  => 'H1',
				'h2'  => 'H2',
	...
	private function add_icon_style_section() {
		...
		$this->add_control( 'icon_align', [
			...
			'options' => [
				'left'  => [
					'title' => esc_html__( 'Start', 'medizin' ),
					'icon'  => 'eicon-h-align-left',
				],
				'right' => [
					'title' => esc_html__( 'End', 'medizin' ),
		...
		$this->start_controls_tab( 'icon_active_color_tab', [
			'label' => esc_html__( 'Active', 'medizin' ),
		] );
		...
		$this->add_responsive_control( 'icon_size', [
			...
			'selectors' => [
				'{{WRAPPER}} .medizin-accordion .accordion-icons' => 'font-size: {{SIZE}}{{UNIT}};',
			],
		] );
		...
		$this->add_responsive_control( 'icon_space', [
			...
			'selectors' => [
				'body:not(.rtl) {{WRAPPER}} .medizin-accordion.medizin-accordion-icon-right .accordion-icons' => 'margin: 0 0 0 {{SIZE}}{{UNIT}};',
				'body:not(.rtl) {{WRAPPER}} .medizin-accordion.medizin-accordion-icon-left .accordion-icons'  => 'margin: 0 {{SIZE}}{{UNIT}} 0 0;',
				'body.rtl {{WRAPPER}} .medizin-accordion.medizin-accordion-icon-right .accordion-icons'       => 'margin: 0 {{SIZE}}{{UNIT}} 0 0;',
				'body.rtl {{WRAPPER}} .medizin-accordion.medizin-accordion-icon-left .accordion-icons'        => 'margin: 0 0 0 {{SIZE}}{{UNIT}};',
			],
		] );
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'medizin-accordion' );
		...
		if ( $has_icon ) {
			$this->add_render_attribute( 'wrapper', 'class', 'medizin-accordion-icon-' . $settings['icon_align'] );
		}
		?>
	...
	protected function _content_template() {
		...
		view.addRenderAttribute( 'wrapper', 'class', 'medizin-accordion' );
		...
		if ( iconHTML.rendered ) {
			view.addRenderAttribute( 'wrapper', 'class', 'medizin-accordion-icon-' + settings.icon_align);
		}
