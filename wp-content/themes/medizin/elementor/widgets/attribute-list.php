...
class Widget_Attribute_List extends Base {
	...
	public function get_title() {
		return esc_html__( 'Attribute List', 'medizin' );
	}
	...
	protected function _register_controls() {
		...
		$repeater->add_control( 'name', [
			...
			'default'     => esc_html__( 'Name', 'medizin' ),
			'label_block' => true,
		] );
		...
		$this->add_control( 'items', [
			...
			'default'     => [
				[
					'name'  => esc_html__( 'Attribute #1', 'medizin' ),
					'value' => esc_html__( 'Value #1', 'medizin' ),
				],
				[
					'name'  => esc_html__( 'Attribute #2', 'medizin' ),
					'value' => esc_html__( 'Value #2', 'medizin' ),
				],
			],
			'title_field' => '{{{ name }}}',
		] );
					...
					<?php if ( $settings['items'] && count( $settings['items'] ) > 0 ) {
						foreach ( $settings['items'] as $key => $attribute ) {
							...
							<div class="item">
								...
								<div class="value"><?php echo wp_kses( $attribute['value'], 'medizin-default' ); ?></div>
							</div>
							<?php
						}
					}
	...
	private function add_styling_section() {
		...
		$this->add_control( 'items_vertical_spacing', [
			'label'      => esc_html__( 'Items Vertical Spacing', 'medizin' ),
		...
		$this->add_control( 'separator_heading', [
			'label'     => esc_html__( 'Separator', 'medizin' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		] );
