...
class Widget_Contact_Form_7 extends Form_Base {
	...
	public function get_title() {
		return esc_html__( 'Contact Form 7', 'medizin' );
	}
	...
	private function get_form_list() {
		...
		if ( $cf7 ) {
			...
			$forms[0] = esc_html__( 'No contact forms found', 'medizin' );
		}
	...
	private function add_content_section() {
		...
		$this->add_control( 'form_id', [
			'label'   => esc_html__( 'Select Form', 'medizin' ),
		...
		$this->add_control( 'style', [
			...
			'default'      => '01',
			'prefix_class' => 'medizin-contact-form-style-',
		] );
	...
	private function add_form_style_section() {
		...
		$this->add_control(
			'column_gap',
			[
				'label'     => esc_html__( 'Columns Gap', 'medizin' ),
		...
		$this->add_control(
			'row_gap',
			[
				'label'     => esc_html__( 'Rows Gap', 'medizin' ),
				'type'      => Controls_Manager::SLIDER,
				'default'   => [
					'size' => 20,
				],
	...
	protected function render() {
		...
		$this->add_render_attribute( 'box', 'class', 'medizin-contact-form-7' );
		?>
