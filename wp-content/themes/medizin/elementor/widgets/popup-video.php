...
class Widget_Popup_Video extends Base {
	...
	public function get_title() {
		return esc_html__( 'Popup Video', 'medizin' );
	}
	...
	private function add_video_section() {
		...
		$this->add_control( 'type', [
			...
			'default' => 'poster',
			'options' => [
				'poster' => esc_html__( 'Poster', 'medizin' ),
				'button' => esc_html__( 'Button', 'medizin' ),
			],
		] );

		$this->add_control( 'video_url', [
			'label'       => esc_html__( 'Video Url', 'medizin' ),
			'description' => esc_html__( 'Input Youtube video url or Vimeo video url. For e.g: "https://www.youtube.com/watch?v=XHOmBV4js_E"', 'medizin' ),
		...
		$this->add_control( 'video_text', [
			'label'         => esc_html__( 'Video Text', 'medizin' ),
			'type'          => Controls_Manager::TEXT,
			'label  _block' => true,
			'condition'     => [
				'type' => 'button',
		...
		$this->add_control( 'video_text_animate', [
			'label'        => esc_html__( 'Text Animate', 'medizin' ),
			'type'         => Controls_Manager::SELECT,
			'options'      => [
				''             => esc_html__( 'None', 'medizin' ),
				'animate-line' => esc_html__( 'Animate Line', 'medizin' ),
			],
			'default'      => '',
			'prefix_class' => 'medizin-text-',
			'condition'    => [
				'type'        => 'button',
				'video_text!' => '',
			],
		...
		$this->add_control( 'position', [
			...
			'prefix_class' => 'medizin-popup-video-icon-position-',
			'toggle'       => false,
			'condition'    => [
				'type'        => 'button',
				'video_text!' => '',
		...
		$this->add_control( 'poster', [
			'label'     => esc_html__( 'Poster Image', 'medizin' ),
		...
		$this->add_control( 'button_type', [
			'label'     => esc_html__( 'Button Type', 'medizin' ),
			'type'      => Controls_Manager::SELECT,
			'default'   => '',
			'options'   => [
				''      => esc_html__( 'Default', 'medizin' ),
				'image' => esc_html__( 'Image', 'medizin' ),
			],
			'separator' => 'before',
		] );

		$this->add_control( 'button_image', [
			'label'     => esc_html__( 'Button Image', 'medizin' ),
			'type'      => Controls_Manager::MEDIA,
			'default'   => [
	...
	private function add_image_style_section() {
		...
		$this->add_responsive_control( 'image_border_width', [
			...
			'selectors' => [
				'{{WRAPPER}} .medizin-image' => 'border-width: {{SIZE}}{{UNIT}}',
			],
		] );
		...
		$this->add_control( 'image_border_color', [
			...
			'selectors' => [
				'{{WRAPPER}} .medizin-image' => 'border-color: {{VALUE}};',
			],
		] );
		...
		$this->add_control( 'hover_image_border_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .video-link:hover .medizin-image' => 'border-color: {{VALUE}};',
			],
		] );

		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			'name'     => 'hover_image_box_shadow',
			'selector' => '{{WRAPPER}} .video-link:hover .medizin-image',
		] );
	...
	protected function render() {
		...
		$this->add_render_attribute( 'link', 'class', 'video-link medizin-box link-secret' );
	...
	private function print_button_image( array $settings ) {
		...
		<div class="video-play video-play-image">
			<?php echo \Medizin_Image::get_elementor_attachment( [
				...
				'attributes' => [
					'alt' => esc_attr__( 'Play Icon', 'medizin' ),
				],
			] ); ?>
		</div>
		<?php
	...
	private function get_default_play_icon() {
		$icon_url = MEDIZIN_ELEMENTOR_ASSETS . '/images/video-play-light.png';
