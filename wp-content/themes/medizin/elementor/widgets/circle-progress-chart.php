...
class Widget_Circle_Progress_Chart extends Base {
	...
	public function get_title() {
		return esc_html__( 'Circle Progress Chart', 'medizin' );
	}
	...
	public function get_script_depends() {
		return [ 'medizin-widget-circle-progress' ];
	}
	...
	private function add_chart_section() {
		$this->start_controls_section( 'chart_section', [
			'label' => esc_html__( 'Chart', 'medizin' ),
		] );

		$this->add_control( 'number', [
			'label'   => esc_html__( 'Number', 'medizin' ),
		...
		$this->add_control( 'size', [
			'label'       => esc_html__( 'Circle Size', 'medizin' ),
			'description' => esc_html__( 'Controls the size of the pie chart circle.', 'medizin' ),
			'type'        => Controls_Manager::NUMBER,
			'default'     => 180,
			'min'         => 100,
			'max'         => 1000,
		...
		$this->add_control( 'unit', [
			'label'       => esc_html__( 'Measuring unit', 'medizin' ),
			'description' => esc_html__( 'Controls the unit of chart.', 'medizin' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '%',
		] );

		$this->add_control( 'reverse', [
			'label' => esc_html__( 'Reverse animation and arc draw', 'medizin' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'line_cap', [
			'label'   => esc_html__( 'Line Cap', 'medizin' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'butt'   => esc_html__( 'Butt', 'medizin' ),
				'round'  => esc_html__( 'Round', 'medizin' ),
		...
		$this->add_control( 'line_width', [
			'label'   => esc_html__( 'Line Width', 'medizin' ),
			'type'    => Controls_Manager::NUMBER,
			'default' => 3,
			'min'     => 1,
			'max'     => 50,
		...
		$this->add_control( 'inner_content_type', [
			'label'   => esc_html__( 'Inner Content', 'medizin' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''     => esc_html__( 'Animate Number', 'medizin' ),
				'text' => esc_html__( 'Custom Text', 'medizin' ),
			],
			'default' => '',
		] );
	...
	private function add_chart_style_section() {
		...
		$this->add_control( 'bar_color', [
			'label'  => esc_html__( 'Bar Color', 'medizin' ),
		...
		$this->add_control( 'track_color', [
			'label'   => esc_html__( 'Track Color', 'medizin' ),
			'type'    => Controls_Manager::COLOR,
			'default' => '#ededed',
		] );

		$this->add_control( 'number_color', [
			'label'     => esc_html__( 'Number Color', 'medizin' ),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
