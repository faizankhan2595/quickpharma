...
class Widget_Google_Map extends Base {
	...
	public function get_title() {
		return esc_html__( 'Advanced Google Map', 'medizin' );
	}
	...
	public function get_script_depends() {
		return [ 'medizin-gmap3', 'medizin-maps', 'medizin-widget-google-map' ];
	}
	...
	private function add_map_options_section() {
		...
		$this->add_control( 'style', [
			'label'   => esc_html__( 'Map Style', 'medizin' ),
		...
		$this->add_control( 'type', [
			'label'   => esc_html__( 'Map Type', 'medizin' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'roadmap'   => esc_html__( 'Roadmap', 'medizin' ),
				'satellite' => esc_html__( 'Satellite', 'medizin' ),
				'hybrid'    => esc_html__( 'Hybrid', 'medizin' ),
				'terrain'   => esc_html__( 'Terrain', 'medizin' ),
			],
			'default' => 'roadmap',
		] );

		$this->add_control( 'zoom_level', [
			'label'       => esc_html__( 'Zoom Level', 'medizin' ),
			'placeholder' => '12',
		...
		$this->add_control( 'scrollwheel', [
			'label'   => esc_html__( 'Mouse Scroll Wheel Zoom', 'medizin' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'draggable', [
			'label'   => esc_html__( 'Draggable', 'medizin' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'map_controls_heading', [
			'label'     => esc_html__( 'Controls', 'medizin' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		] );

		$this->add_control( 'zoom_control', [
			'label' => esc_html__( 'Zoom Control', 'medizin' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'scale_control', [
			'label' => esc_html__( 'Scale Control', 'medizin' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'fullscreen_control', [
			'label' => esc_html__( 'Fullscreen Control', 'medizin' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'street_view_control', [
			'label' => esc_html__( 'Street View Control', 'medizin' ),
			'type'  => Controls_Manager::SWITCHER,
		] );
	...
	private function add_map_markers_section() {
		$this->start_controls_section( 'markers_section', [
			'label' => esc_html__( 'Markers', 'medizin' ),
		] );

		$this->add_control( 'marker_style', [
			'label'       => esc_html__( 'Marker Style', 'medizin' ),
			'description' => esc_html__( 'Select default style for all markers', 'medizin' ),
			'type'        => Controls_Manager::SELECT,
			'options'     => [
				'icon'   => esc_html__( 'Icon', 'medizin' ),
				'signal' => esc_html__( 'Signal', 'medizin' ),
			],
			'default'     => 'icon',
		] );

		$this->add_control( 'marker_icon', [
			'label'     => esc_html__( 'Marker Icon', 'medizin' ),
			...
			'classes'   => 'medizin-control-media-auto',
		] );
		...
		$repeater->start_controls_tab( 'marker_content_tab', [
			'label' => esc_html__( 'Marker', 'medizin' ),
		] );

		$repeater->add_control( 'address', [
			'label'       => esc_html__( 'Address or Coordinate', 'medizin' ),
		...
		$repeater->start_controls_tab( 'marker_popup_tab', [
			'label' => esc_html__( 'Popup', 'medizin' ),
		] );

		$repeater->add_control( 'overlay_display', [
			'label'   => esc_html__( 'Display', 'medizin' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''       => esc_html__( 'Hover', 'medizin' ),
				'always' => esc_html__( 'Always', 'medizin' ),
			],
			'default' => '',
		] );
		...
		$repeater->add_control( 'image', [
			...
			'description' => esc_html__( 'Choose an image that displays on left info box', 'medizin' ),
			'type'        => Controls_Manager::MEDIA,
			'classes'     => 'medizin-control-media-auto',
		] );
		...
		$this->add_group_control( Group_Control_Image_Size::get_type(), [
			'label'     => esc_html__( 'Popup Image Size', 'medizin' ),
	...
	private function add_map_markers_style_section() {
		$this->start_controls_section( 'markers_popup_style_section', [
			'label' => esc_html__( 'Markers Popup', 'medizin' ),
			'tab'   => Controls_Manager::TAB_STYLE,
		] );

		$this->add_control( 'markers_popup_wrapper_heading', [
			'label' => esc_html__( 'Wrapper', 'medizin' ),
			'type'  => Controls_Manager::HEADING,
		] );
		...
		$this->add_responsive_control( 'markers_popup_padding', [
			...
			'size_units' => [ 'px', '%', 'em' ],
			'selectors'  => [
				'{{WRAPPER}} .medizin-map-overlay-info' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );

		$this->add_responsive_control( 'markers_popup_width', [
			'label'      => esc_html__( 'Width', 'medizin' ),
			'type'       => Controls_Manager::SLIDER,
			'default'    => [
				'unit' => 'px',
			],
			...
			'selectors'  => [
				'{{WRAPPER}} .medizin-map-overlay-content' => 'width: {{SIZE}}{{UNIT}};',
			],
		] );
	...
	private function get_overlay_value( $settings, $marker, $position ) {
		...
		$this->add_render_attribute( $item_key, 'class', [
			'medizin-map-overlay',
			"medizin-map-overlay-{$marker['overlay_display']}",
		...
		if ( $marker['title'] !== '' || $marker['content'] !== '' ) {
			$info_template .= '<div class="medizin-map-overlay-template"><div class="medizin-map-overlay-content"><div class="medizin-map-overlay-info">';

			if ( isset( $marker['image'] ) ) {
				$size  = \Medizin_Image::elementor_parse_image_size( $settings, '80x9999', 'popup_image_size' );
				$image = \Medizin_Image::get_attachment_by_id( array(
					'id'   => $marker['image']['id'],
					'size' => $size,
				) );
	...
	private function get_map_style() {
		return [
			'no_label_bright_colors'  => esc_html__( 'No Label Bright Colors', 'medizin' ),
			'grayscale'               => esc_html__( 'Grayscale', 'medizin' ),
			'subtle_grayscale'        => esc_html__( 'Subtle Grayscale', 'medizin' ),
			'apple_paps_esque'        => esc_html__( 'Apple Maps-esque', 'medizin' ),
			'pale_dawn'               => esc_html__( 'Pale Dawn', 'medizin' ),
			'midnight_commander'      => esc_html__( 'Midnight Commander', 'medizin' ),
			'blue_water'              => esc_html__( 'Blue Water', 'medizin' ),
			'retro'                   => esc_html__( 'Retro', 'medizin' ),
			'paper'                   => esc_html__( 'Paper', 'medizin' ),
			'ultra_light_with_labels' => esc_html__( 'Ultra Light with Labels', 'medizin' ),
			'shades_of_grey'          => esc_html__( 'Shades Of Grey', 'medizin' ),
			'easy_light'              => esc_html__( 'Easy Light', 'medizin' ),
			'wy'                      => esc_html__( 'WY', 'medizin' ),
		];
	}
	...
	private function get_default_marker_icon() {
		$icon_url = MEDIZIN_ELEMENTOR_ASSETS . '/images/map-marker.png';
