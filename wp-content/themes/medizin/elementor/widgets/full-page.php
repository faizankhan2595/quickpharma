...
class Widget_Full_Page extends Base {
	...
	public function __construct( $data = [], $args = null ) {
		...
		wp_register_style( 'medizin-widget-fullpage', MEDIZIN_THEME_URI . '/assets/libs/fullpage/fullpage.min.css', null, '3.0.5' );

		// Fullpage.
		wp_register_script( 'fullpage', MEDIZIN_THEME_URI . '/assets/libs/fullpage/fullpage.js', array(), null, true );
		//wp_register_script( 'fullpage-extensions', MEDIZIN_THEME_URI . '/assets/libs/fullpage/fullpage.extensions.min.js', array(), null, true );
		wp_register_script( 'scrolloverflow', MEDIZIN_THEME_URI . '/assets/libs/fullpage/vendors/scrolloverflow.js', array(), null, true );
		wp_register_script( 'medizin-widget-fullpage', MEDIZIN_ELEMENTOR_URI . '/assets/js/widgets/widget-fullpage.js', array(
			'elementor-frontend',
			'fullpage',
			'scrolloverflow',
		), null, true );
	...
	public function get_title() {
		return esc_html__( 'Fullpage', 'medizin' );
	}
	...
	public function get_script_depends() {
		return [ 'medizin-widget-fullpage' ];
	}
	...
	private function add_slides_section() {
		$this->start_controls_section( 'slides_section', [
			'label' => esc_html__( 'Slides', 'medizin' ),
		] );
		...
		$repeater->start_controls_tab( 'repeater_slides_content_tab', [ 'label' => esc_html__( 'Content', 'medizin' ) ] );
		...
		$repeater->add_control( 'template_id', [
			'label'        => esc_html__( 'Choose Template', 'medizin' ),
		...
		$repeater->add_control( 'anchor', [
			'label' => esc_html__( 'Anchor', 'medizin' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$repeater->add_control( 'skin', [
			'label'       => esc_html__( 'Skin', 'medizin' ),
			'description' => esc_html__( 'Controls the skin of pagination & header when view this section.', 'medizin' ),
			'type'        => Controls_Manager::CHOOSE,
			'options'     => [
				'light' => [
					'text' => esc_html__( 'Light', 'medizin' ),
				],
				'dark'  => [
					'text' => esc_html__( 'Dark', 'medizin' ),
				],
			],
			'toggle'      => false,
			'default'     => 'light',
		...
		$repeater->add_control( 'auto_height', [
			'label' => esc_html__( 'Auto Height', 'medizin' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$repeater->end_controls_tab();

		$repeater->start_controls_tab( 'repeater_slides_background_tab', [ 'label' => esc_html__( 'Background', 'medizin' ) ] );
		...
		$this->add_control( 'slides', [
			...
			'default'     => [
				[
					'title' => esc_html__( 'Slide #1', 'medizin' ),
				],
				[
					'title' => esc_html__( 'Slide #2', 'medizin' ),
				],
			],
			'title_field' => '{{{ title }}}',
		] );
	...
	private function add_fullpage_options_section() {
		$this->start_controls_section( 'fullpage_options_section', [
			'label' => esc_html__( 'Fullpage Options', 'medizin' ),
		] );

		$this->add_control( 'dots', [
			'label' => esc_html__( 'Dots Enable', 'medizin' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'dots_alignment', [
			'label'     => esc_html__( 'Dots Alignment', 'medizin' ),
			'type'      => Controls_Manager::CHOOSE,
			'options'   => [
				'left'  => [
		...
		$this->add_control( 'numbers', [
			'label' => esc_html__( 'Numbers Enable', 'medizin' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'sharing', [
			'label' => esc_html__( 'Share Enable', 'medizin' ),
			'type'  => Controls_Manager::SWITCHER,
		] );
	...
	private function print_share( array $settings ) {
		...
		<div id="full-page-sharing" class="full-page-share-list">
			<div class="stalk">
				<?php esc_html_e( 'Stalk', 'medizin' ); ?>
			</div>
			<?php \Medizin_Templates::get_sharing_list( array(
	...
	private function print_slide_numbers( array $settings ) {
		...
		<div id="full-page-numbers" class="full-page-numbers">
			...
			<div class="title">
				<?php esc_html_e( 'Projects', 'medizin' ); ?>
			</div>
		</div>
		<?php
	}
