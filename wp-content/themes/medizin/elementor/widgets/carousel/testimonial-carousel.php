...
class Widget_Testimonial_Carousel extends Static_Carousel {
	...
	public function get_title() {
		return esc_html__( 'Testimonial Carousel', 'medizin' );
	}
	...
	private function add_layout_section() {
		...
		$this->add_control( 'style', [
			...
			'prefix_class' => 'medizin-testimonial-style-',
		] );
		...
		$this->add_control( 'layout', [
			...
			'options'      => [
				...
				'image-top'     => esc_html__( 'Image Top Overlap', 'medizin' ),
				'image-top-02'  => esc_html__( 'Image Top', 'medizin' ),
				'image-above'   => esc_html__( 'Image Above', 'medizin' ),
				'image-left'    => esc_html__( 'Image Left', 'medizin' ),
			],
			'render_type'  => 'template',
			'prefix_class' => 'layout-',
		] );
		...
		$this->add_control( 'image_position', [
			'label'        => esc_html__( 'Info Position', 'medizin' ),
		...
		$this->add_control( 'cite_layout', [
			'label'        => esc_html__( 'Cite Layout', 'medizin' ),
			'label_block'  => false,
			'type'         => Controls_Manager::CHOOSE,
			'default'      => 'block',
			'options'      => [
			...
			'prefix_class' => 'medizin-testimonial-cite-layout-',
		] );
	...
	protected function get_repeater_defaults() {
		...
		return [
			[
				'content'  => esc_html__( 'I am slide content. Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'medizin' ),
				'name'     => esc_html__( 'Frankie Kao', 'medizin' ),
				'position' => esc_html__( 'Web Design', 'medizin' ),
	...
	protected function update_slider_settings( $settings, $slider_settings ) {
		if ( 'image-above' === $settings['layout'] ) {
			$slider_settings['class'][]            = 'medizin-main-swiper';
	...
	protected function before_slider() {
		...
		$this->add_render_attribute( '_wrapper', 'class', 'medizin-swiper-linked-yes' );

		$testimonial_thumbs_template = '';

		foreach ( $settings['slides'] as $slide ) :
			if ( $slide['image']['url'] ) :
				$testimonial_thumbs_template .= '<div class="swiper-slide"><div class="post-thumbnail"><div class="image">' . \Medizin_Image::get_elementor_attachment( [
						'settings'       => $slide,
						'image_size_key' => 'image_size',
					] ) . '</div></div></div>';
			endif;
		...
		?>
		<div class="tm-swiper tm-slider-widget medizin-testimonial-pagination style-01 medizin-thumbs-swiper"
		     data-lg-items="3"
		     data-lg-gutter="30"
