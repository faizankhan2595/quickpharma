...
class Widget_Product_Carousel_Countdown extends Posts_Carousel_Base
{
	...
	{
		return esc_html__('Product Carousel Countdown', 'medizin');
	}
	...
	{
		return ['medizin-product-carousel-countdown'];
	}
	...
	private function add_layout_section()
	{
		...
		$this->add_control('style', [
				...
				'options' => array(
						...
						'grid-03' => esc_html__('Style 03', 'medizin'),
				),
				'default' => 'grid-01',
	...
	private function add_countdown_section()
	{
		$this->start_controls_section('countdown_section', [
				'label' => esc_html__('Countdown', 'medizin'),
		]);
		...
		$this->add_control('countdown_type', [
				...
				'type' => Controls_Manager::SELECT,
				'options' => [
						'due_date' => esc_html__('Due Date', 'medizin'),
						'daily' => esc_html__('Daily', 'medizin'),
				],
				'default' => 'daily',
		]);

		$this->add_control('due_date', [
				'label' => esc_html__('Due Date', 'medizin'),
				'type' => Controls_Manager::DATE_TIME,
				'default' => gmdate('Y-m-d H:i', strtotime('+1 month') + (get_option('gmt_offset') * HOUR_IN_SECONDS)),
			/* translators: %s: Time zone. */
				'description' => sprintf(__('Date set according to your timezone: %s.', 'medizin'), Utils::get_timezone_string()),
				'condition' => [
		...
		$this->add_control('countdown_title', [
				...
				'default' => esc_html__('Daily Deal Of The Day', 'medizin'),
		]);

		$this->add_control('product_carousel_countdown_style', [
				'label' => esc_html__('Countdown Style', 'medizin'),
				'type' => Controls_Manager::SELECT,
				'options' => array(
						'product-carousel-countdown-style-01' => esc_html__('Style 01', 'medizin'),
						'product-carousel-countdown-style-02' => esc_html__('Style 02', 'medizin'),
				),
	...
	public function before_slider()
	{
		...
		echo '<div class="medizin-product-carousel-countdown">';
	...
	private function print_countdown()
	{
		...
		$days_text = isset($days) && $days !== '' ? $days : esc_html__('Days', 'medizin');
		$hours_text = isset($hours) && $hours !== '' ? $hours : esc_html__('Hours', 'medizin');
		$minutes_text = isset($minutes) && $minutes !== '' ? $minutes : esc_html__('Minutes', 'medizin');
		$seconds_text = isset($seconds) && $seconds !== '' ? $seconds : esc_html__('Seconds', 'medizin');
		...
		<div class="product-countdown-header  " >
			...
			<div class="countdown-wrap">
				<div class="countdown-label"><?php esc_html_e('End in:', 'medizin'); ?></div>
