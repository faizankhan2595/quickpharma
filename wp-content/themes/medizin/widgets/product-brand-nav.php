...
if ( ! class_exists( 'Medizin_WP_Widget_Product_Brand_Nav' ) ) {
	class Medizin_WP_Widget_Product_Brand_Nav extends Medizin_Widget {
		public function __construct() {

			$this->widget_id          = 'medizin-wp-widget-product-brand-nav';
			$this->widget_cssclass    = 'medizin-wp-widget-product-brand-nav';
			$this->widget_name        = esc_html__( '[Medizin] Product Brand Layered Nav', 'medizin' );
			$this->widget_description = esc_html__( 'Shows brands in a widget which lets you narrow down the list of products when viewing products.', 'medizin' );
			$this->settings           = array(
				'title'        => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Brands', 'medizin' ),
					'label' => esc_html__( 'Title', 'medizin' ),
				),
				'display_type' => array(
					'type'    => 'select',
					'std'     => 'list',
					'label'   => esc_html__( 'Display type', 'medizin' ),
					'options' => array(
						'list'     => esc_html__( 'List', 'medizin' ),
						'dropdown' => esc_html__( 'Dropdown', 'medizin' ),
					),
				),
			);
		...
		protected function layered_nav_dropdown( $terms, $taxonomy, $depth = 0 ) {
			...
			if ( $taxonomy !== $this->get_current_taxonomy() ) {
				...
				if ( 0 == $depth ) {
					...
					echo '<option value="">' . __( 'Any Brand', 'medizin' ) . '</option>';
				}
